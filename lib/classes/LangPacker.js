...
class LangPacker {
    ...
    do() {
        ...
        let isLangDir = this.input.endsWith('/.lang') || this.input.endsWith('/.lang/');
    ...
    checkDir(dir, path, res, isLangDir) {
        ...
        for (var i in files) {
            ...
            if (fs.statSync(name).isDirectory()) {
                this.checkDir(name, `${path}/${files[i]}`, res, isLangDir || files[i] === '.lang');
            } else if (isLangDir) {
        ...
        if (_.isObject(o)) {
            ...
            if (_.size(objects) > 0) {
                _.forEach(objects, (obj, key) => {
                    res += this.buildObject(obj, `${path}${path.endsWith(":") ? "" : "." }${key}`);
                });
            }
        }
