    ...
    parseFile(file) {
        ...
        for (const line of lines) {
            if (line.startsWith("$LANGUAGE")) {
                ...
                if (this.skip && path) {
                    let prevPath = path;
                    while (prevPath.length > 1) {
                        const parentPath = prevPath.slice(0, -1);
                        const parentDir = parentPath.length ? _.get(res, parentPath) : res;
                        if (parentDir) {
                            const key = prevPath.slice(-1)[0];
                            if (Object.prototype.hasOwnProperty.call(parentDir, key)) {
                                const value = parentDir[key];
                                const isEmpty = !value || (Array.isArray(value) && !value.length) || !Object.keys(value).length;
                                if (isEmpty) delete parentDir[key];
                                else break;
                            }
                        }
                        prevPath = parentPath;
                ...
                const [__, ...rest] = l.split("\t");
                const tempPath = rest.join("");

                const [k, p] = tempPath.split(":");

                path = [k];

                if (p) path = [...path, ...p.split(".")];
